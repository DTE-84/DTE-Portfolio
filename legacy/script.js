document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(
      ".section-title, .project-card, .about-content, .about-reveal",
    )
    .forEach((el) => {
      observer.observe(el);
    });
    ...
    orbs.forEach((orb) => {
      ...
      if (wrapper) {
        wrapper.classList.toggle(
          "active",
          wrapper.getAttribute("data-section") === currentSection,
        );
      }
    });
  });
  ...
  if (hamburger && navLinksList) {
    hamburger.addEventListener("click", () => {
      ...
      document.body.style.overflow = navLinksList.classList.contains("active")
        ? "hidden"
        : "";
    });
...
var ParticleEngine = (function () {
  ...
  function ParticleEngine(canvas_id) {
    ...
    var isYellowTheme =
      document.documentElement.getAttribute("data-theme") === "yellow";
